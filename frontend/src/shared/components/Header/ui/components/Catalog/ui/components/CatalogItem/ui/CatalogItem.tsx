  ...
  return (
    <div className={clsx(css.catalog_item, {[css.open]: isExpanded})}>
      <ul className={clsx(css.catalog_item_wrapper, css[deviceType])} id={id}>
        <li 
          className={clsx(css.catalog_item_title, css[deviceType], { [css.select]: isSelect })} 
          onClick={handleTitleClick} // Клик на заголовок
        >
          {title}
        </li>
        <ul className={clsx(css.links_list, { [css.hidden]: !isExpanded })}> {/* Скрываем на основе props */}
          {(links || []).map((link) => (  
